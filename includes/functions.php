...
function getTotalQuantity() {
    ...
    if (isset($_SESSION['cart'])) {
        foreach ($_SESSION['cart'] as $item_type => $cart_items) {
...
function getTotalWishlistQuantity() {
    $totalQuantity = 0;

    if (isset($_SESSION['yeuthich'])) {
        foreach ($_SESSION['yeuthich'] as $item_type => $product_ids) {
            $totalQuantity += count($product_ids);  // Cộng thêm số lượng sản phẩm trong mỗi loại
        }
    }
